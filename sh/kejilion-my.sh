...
linux_tools() {
  ...
  while true; do
    ...
    echo -e "${gl_kjlan}13.  ${gl_bai}ncdu,duf 磁盘占用查看工具             ${gl_kjlan}14.  ${gl_bai}fzf 全局搜索工具"
    echo -e "${gl_kjlan}15.  ${gl_bai}vim 文本编辑器                    ${gl_kjlan}16.  ${gl_bai}nano 文本编辑器 ${gl_huang}★${gl_bai}"
    echo -e "${gl_kjlan}17.  ${gl_bai}git 版本控制系统                    ${gl_kjlan}18.  ${gl_bai}zsh zsh命令行 ${gl_huang}★${gl_bai}"
    echo -e "${gl_kjlan}------------------------"
    echo -e "${gl_kjlan}21.  ${gl_bai}-                      ${gl_kjlan}22.  ${gl_bai}-"
    echo -e "${gl_kjlan}26.  ${gl_bai}-                  ${gl_kjlan}27.  ${gl_bai}-"
    echo -e "${gl_kjlan}28.  ${gl_bai}-"
    echo -e "${gl_kjlan}------------------------"
    ...
    31)
      clear
      send_stats "全部安装"
      install curl wget sudo socat htop iftop unzip tar tmux ffmpeg btop ranger ncdu duf fzf cmatrix sl bastet nsnake ninvaders vim nano git
      ;;

    32)
      clear
      send_stats "全部安装（不含游戏和屏保）"
      install curl wget sudo socat htop iftop unzip tar tmux ffmpeg btop ranger ncdu duf fzf vim nano git
      ;;
...
linux_panel() {
  ...
  while true; do
    ...
    echo -e "${gl_kjlan}19.  ${gl_bai}雷池WAF防火墙面板                   ${gl_kjlan}20.  ${gl_bai}portainer容器管理面板 admin-adminP@ssw0rd"
    echo -e "${gl_kjlan}------------------------"
    ...
    echo -e "${gl_kjlan}------------------------"
    echo -e "${gl_kjlan}51.  ${gl_bai}PVE开小鸡面板(虚拟机)"
    echo -e "${gl_kjlan}------------------------"
    echo -e "${gl_kjlan}0.   ${gl_bai}返回主菜单"
    echo -e "${gl_kjlan}------------------------${gl_bai}"
    read -e -p "请输入你的选择: " sub_choice
    ...
    21)
      work='/opt/docker/vscode-web'
      docker_name="vscode-web"
      docker_img="ghcr.io/coder/coder"
      docker_port=8180
      ...
      echo -e "
              ...
              services:
                  vscode-web:
                      ...
                      restart: unless-stopped
                      volumes:
                          - $work/alist_conf:/opt/alist/data
                          - $work/storage:/opt/alist/storage
                          - /var/run/docker.sock:/var/run/docker.sock
                      ports:
                          - 37080:7080
                      environment:
                          - CODER_ADDRESS: 0.0.0.0:7080
                          - CODER_ACCESS_URL: https://code.2020224.xyz
                          - TZ=Asia/Shanghai
...
linux_Settings() {
  ...
  while true; do
    ...
    echo -e "${gl_kjlan}-----------自定义------------"
    echo -e "${gl_kjlan}70.  ${gl_bai}sshd_config安全设置                ${gl_kjlan}71.  ${gl_bai} 允许portainer Agent配置 ${gl_huang}★${gl_bai}"
    echo -e "${gl_kjlan}72.  ${gl_bai}设置常用alias                      ${gl_kjlan}73.  ${gl_bai} 设置 docker ps 显示 format ${gl_huang}★${gl_bai}"
    # echo -e "${gl_kjlan}74.  ${gl_bai}留言板                             ${gl_kjlan}75.  ${gl_bai}一条龙系统调优 ${gl_huang}★${gl_bai}"
    echo -e "${gl_kjlan}------------------------"
...
EOF
    ...
    70)
      ...
      *)
      ...
      ;;
    71)
      echo "portainer Agent 设置"
      read -e -p "确定继续吗？(Y/N): " choice

      case "$choice" in
      [Yy])
        clear
        enable_portainer_connection
        ;;
      [Nn])
        echo "已取消"
        ;;
...
kejilion_sh() {
  while true; do
    ...
    echo -e "${gl_kjlan}15.  ${gl_bai}安装docker应用"
...
# 开启端口portainer链接docker
enable_portainer_connection() {
  # 备份
  cp /usr/lib/systemd/system/docker.service /usr/lib/systemd/system/docker.service.bak
  read -e -p "请输入  portainer Agent 端口号(注意设置防火墙): " dpa_port
  sed -i 's/^#\?ExecStart.*/ExecStart=\/usr\/bin\/dockerd -H fd:\/\/ --containerd=\/run\/containerd\/containerd.sock -H tcp:\/\/0.0.0.0:$dpa_port -H unix:\/\/var\/run\/docker.sock/g' /usr/lib/systemd/system/docker.service
  sudo systemctl daemon-reload
  sudo systemctl restart docker
  sudo ufw deny $dpa_port/tcp
}
...
set_default_alias() {
  ...
  echo "
    ...
    alias keygen='ssh-keygen -t ed25519 -C '' -P ""'
    " >>~/.zshrc
  source ~/.zshrc
}

# 设置 docker ps 显示
# json 显示 docker ps --format '{{json .}}' | jq .
set_docker_display() {
  mkdir ~/.docker
  cat >~/.docker/config.json <<EOF
        {
           "experimental": "enabled",
           "psFormat": "table {{.ID}}\t{{.Names}}\t{{.Status}}\t{{.Size}}\t{{.Networks}}\t{{printf \"%.40s\" .Ports}}"
        }
EOF
}
